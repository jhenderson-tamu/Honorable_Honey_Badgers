...
def build_table_popup(parent, title, df, export_name="expenses"):
    ...
    specs = {
        ...
        "Amount": {"width": 110, "anchor": "w", "stretch": False},
    ...
    def _fmt_date(dt): return dt.strftime("%Y-%m-%d") if pd.notnull(dt) else ""
    def _fmt_amt(val): return f"${val:,.2f}" if pd.notnull(val) else "$0.00"
