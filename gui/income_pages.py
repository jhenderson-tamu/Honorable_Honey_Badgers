...
class IncomePages:
    ...
    def create_income_page(self):
        """Create the main income page with options"""
        # Clear any existing widgets
        for widget in self.parent_frame.winfo_children():
            widget.destroy()
        
        # Title label
        ttk.Label(self.parent_frame, text=f"Income Options for {self.username}",
        ...
        # Income Manual Entry Button
        ttk.Button(frame, text="Income Manual Entry", bootstyle="primary", width=20,
                   command=self.income_entry_page).pack(pady=10)
        
        # Import Income Button
        ttk.Button(frame, text="Import Income", bootstyle="primary", width=20,
                   command=self.import_income_csv).pack(pady=10)
        
        # View Income Button
        ttk.Button(frame, text="View Income", bootstyle="primary", width=20,
                   command=self.view_income_page).pack(pady=10)
        
        # Remove Income Button
        ttk.Button(frame, text="Remove Income", bootstyle="primary", width=20,
                   command=self.remove_income_page).pack(pady=10)
        
        # Close button
        ttk.Button(frame, text="Close Income Page", bootstyle="danger", width=20,
                   command=self.close_income_page).pack(pady=20)
    
    def close_income_page(self):
        """Close the income page"""
        for widget in self.parent_frame.winfo_children():
            widget.destroy()
    
    def income_entry_page(self):
        """Create income manual entry page"""
        for widget in self.parent_frame.winfo_children():
            widget.destroy()
        
        ttk.Label(self.parent_frame, text=f"Manual Income Entry for {self.username}",
                  font=("Helvetica", 14, "bold")).pack(pady=15)
        ...
        date_entry = ttk.DateEntry(form_frame, bootstyle="info", dateformat="%Y-%m-%d")
        ...
        def add_category():
            ...
            if not new_cat:
                show_notification("Category name cannot be empty.", type="error")
                return
            ...
            if success:
                ...
                categories = self.finance_ops.load_income_categories()
        ...
        def save_income():
            ...
            success, message = self.finance_ops.add_income(date, desc, category, amount, self.username)
            show_notification(message, type="info" if success else "error")
        ...
        ttk.Button(form_frame, text="Save Income", bootstyle="success",
                   command=save_income).pack(pady=15)
        ttk.Button(form_frame, text="Back to Income", bootstyle="danger",
                   command=self.create_income_page).pack(pady=10)
    
    def import_income_csv(self):
        """Create income import page"""
        for widget in self.parent_frame.winfo_children():
            widget.destroy()
        
        ttk.Label(self.parent_frame, text=f"Import Income CSV for {self.username}",
                  font=("Helvetica", 14, "bold")).pack(pady=15)
        ...
        file_label = ttk.Label(import_frame, textvariable=selected_file, foreground="blue")
        ...
        browse_btn = ttk.Button(import_frame, text="Browse...", bootstyle="primary",
                                command=open_file_dialog)
        browse_btn.pack(pady=10)
        
        import_btn = ttk.Button(import_frame, text="Import", bootstyle="success",
                                command=import_csv, state=DISABLED)
        import_btn.pack(pady=5)
        
        ttk.Button(import_frame, text="Back to Income", bootstyle="danger",
                   command=self.create_income_page).pack(pady=10)
    
    def view_income_page(self):
        """Create view income page"""
        for widget in self.parent_frame.winfo_children():
            widget.destroy()
        
        ttk.Label(self.parent_frame, text=f"View Income for {self.username}",
                  font=("Helvetica", 14, "bold")).pack(pady=15)
        ...
        if not income:
            ...
            ttk.Button(self.parent_frame, text="Back to Income", bootstyle="danger",
                       command=self.create_income_page).pack(pady=5)
            return
    ...
    def remove_income_page(self):
        """Create remove income page"""
        for widget in self.parent_frame.winfo_children():
            widget.destroy()
        
        ttk.Label(self.parent_frame, text=f"Remove Income for {self.username}",
                  font=("Helvetica", 14, "bold")).pack(pady=15)
        ...
        def delete_selected():
            ...
            response = Messagebox.okcancel(
                f"Delete income: {item[0]} (${float(item[3]):.2f}) in {item[2]} from {item[1]}?",
                "Confirm Delete")
            if response != "OK":
                return
        ...
        ttk.Button(self.parent_frame, text="Delete Selected Income",
