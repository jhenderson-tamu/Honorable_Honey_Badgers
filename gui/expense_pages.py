...
class ExpensePages:
    ...
    def create_expenses_page(self):
        """Create the main expenses page with options"""
        ...
        # Manual Entry Button
        ttk.Button(frame, text="Manual Entry", bootstyle="primary", width=20,
                   command=self.expense_entry_page).pack(pady=10)

        # Import Expense Button
        ttk.Button(frame, text="Import Expense", bootstyle="primary", width=20,
                   command=self.import_expenses_csv).pack(pady=10)

        # View Expenses Button
        ttk.Button(frame, text="View Expenses", bootstyle="primary", width=20,
                   command=self.view_expenses_page).pack(pady=10)

        # Remove Expenses Button
        ttk.Button(frame, text="Remove Expenses", bootstyle="primary", width=20,
                   command=self.remove_expenses_page).pack(pady=10)

        # Close button
        ttk.Button(frame, text="Close Expense Page", bootstyle="danger",
                   width=20,
                   command=self.close_expense_page).pack(pady=20)

    def close_expense_page(self):
        """Close the expense page"""
        for widget in self.parent_frame.winfo_children():
            widget.destroy()

    def expense_entry_page(self):
        """Create expense manual entry page"""
        for widget in self.parent_frame.winfo_children():
            widget.destroy()
        ...
        date_entry = ttk.DateEntry(form_frame, bootstyle="info",
                                   dateformat="%Y-%m-%d")
        ...
        # Notification label
        notification_label = ttk.Label(form_frame, text="", foreground="green",
        ...
        def show_notification(message, type="info"):
            ...
            notification_label.after(3000,
                                     lambda: notification_label.config(text=""))

        def add_category():
            new_cat = new_category_entry.get().strip()
            if not new_cat:
                show_notification("Category name cannot be empty.",
                                  type="error")
                return

            success, message = self.finance_ops.add_expense_category(new_cat)
            show_notification(message, type="info" if success else "error")

            if success:
                new_category_entry.delete(0, "end")
                categories = self.finance_ops.load_expense_categories()
        ...
        def save_expense():
            ...
            if not date or not desc or not amount or not category or category == "Select Category":
                show_notification("All fields are required!", type="error")
                return

            try:
                amount = float(amount)
            except ValueError:
                show_notification("Amount must be a number.", type="error")
                return

            success, message = self.finance_ops.add_expense(date, desc,
                                                            category, amount,
                                                            self.username)
            show_notification(message, type="info" if success else "error")
        ...
        # Buttons
        ttk.Button(form_frame, text="Add Category", bootstyle="info",
                   command=add_category).pack(pady=5)
        ttk.Button(form_frame, text="Save Expense", bootstyle="success",
                   command=save_expense).pack(pady=15)
        ttk.Button(form_frame, text="Back to Expenses", bootstyle="danger",
                   command=self.create_expenses_page).pack(pady=5)

    def import_expenses_csv(self):
        """Create expense import page"""
        for widget in self.parent_frame.winfo_children():
            widget.destroy()
        ...
        ttk.Label(import_frame,
                  text="Select CSV file to import your expenses").pack(
            anchor="w", pady=5)

        selected_file = StringVar()
        selected_file.set("No file selected")

        file_label = ttk.Label(import_frame, textvariable=selected_file,
                               foreground="blue")
        ...
        browse_btn = ttk.Button(import_frame, text="Browse...",
                                bootstyle="primary",
                                command=open_file_dialog)
        browse_btn.pack(pady=10)

        import_btn = ttk.Button(import_frame, text="Import",
                                bootstyle="success",
                                command=import_csv, state=DISABLED)
        import_btn.pack(pady=5)

        ttk.Button(import_frame, text="Back to Expenses", bootstyle="danger",
                   command=self.create_expenses_page).pack(pady=10)

    def view_expenses_page(self):
        """Create view expenses page"""
        for widget in self.parent_frame.winfo_children():
            widget.destroy()

        ttk.Label(self.parent_frame, text=f"View Expenses for {self.username}",
                  font=("Helvetica", 14, "bold")).pack(pady=15)

        expenses = self.finance_ops.get_user_expenses(self.username)

        if not expenses:
            ttk.Label(self.parent_frame, text="No Expenses to view.").pack(
                pady=10)
            ttk.Button(self.parent_frame, text="Back to Expenses",
                       bootstyle="danger",
                       command=self.create_expenses_page).pack(pady=5)
            return
        ...
        tree = ttk.Treeview(self.parent_frame, columns=columns, show="headings",
                            height=10)
        ...
        for col in columns:
            ...
            tree.column(col, anchor="center" if col != "Description" else "w",
                        width=10 if col in ["ID", "Date",
                                            "Amount"] else 25 if col == "Category" else 50)
    ...
    def remove_expenses_page(self):
        ...
        if not expenses:
            ttk.Label(self.parent_frame, text="No expenses to remove.").pack(
                pady=10)
            ttk.Button(self.parent_frame, text="Back to Expenses",
                       bootstyle="danger",
                       command=self.create_expenses_page).pack(pady=5)
        ...
        def delete_selected():
            ...
            if not selected:
                Messagebox.show_error("Please select an expense to delete.",
                                      "Error")
                return
            ...
            response = Messagebox.okcancel(
                f"Delete expense: {item[0]} (${float(item[3]):.2f}) in {item[2]} from {item[1]}?",
                "Confirm Delete")
            if response != "OK":
                return
        ...
        ttk.Button(self.parent_frame, text="Delete Selected Expense",
                   command=delete_selected, bootstyle="danger").pack(pady=5)
        ttk.Button(self.parent_frame, text="Back to Expenses",
                   bootstyle="danger",
                   command=self.create_expenses_page).pack(pady=5)